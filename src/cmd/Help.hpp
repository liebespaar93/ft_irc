...
public:
	...
	void ft_recv(std::vector<std::string> msg)
	{
		...
		this->ft_send();
		this->_send_msg = RPL_HELPTXT(this->_client, this->_subject, "-  LIST  - \"/list [empty] or [-c channel_name]\" : list channel if there is an option or users in the channel.");
		this->ft_send();
		...
		this->ft_send();
		this->_send_msg = RPL_HELPTXT(this->_client, this->_subject, "-  KICK  - \"/kick user_name\" : kick the user out of the channel.");
		this->ft_send();
		this->_send_msg = RPL_HELPTXT(this->_client, this->_subject, "- INVITE - \"/invite user_name\" : invite the user.");
		this->ft_send();
		...
		this->ft_send();
		this->_send_msg = RPL_HELPTXT(this->_client, this->_subject, "           \"/mode i\" : set/remove invite-only channel.");
		this->ft_send();
		this->_send_msg = RPL_HELPTXT(this->_client, this->_subject, "           \"/mode t\" : set/remove the restrictions of the TOPIC command to channel operators.");
		this->ft_send();
		this->_send_msg = RPL_HELPTXT(this->_client, this->_subject, "           \"/mode k (-set password) or (-rm)\" : set/remove the channel key.");
		this->ft_send();
		this->_send_msg = RPL_HELPTXT(this->_client, this->_subject, "           \"/mode o user_name\" : give/take channel operator privilege to user_name.");
		this->ft_send();
		this->_send_msg = RPL_HELPTXT(this->_client, this->_subject, "           \"/mode l number\" : set/remove the user limit to channel if number is zero, the limitation is of");
		this->ft_send();
